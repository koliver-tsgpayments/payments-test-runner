...
locals {
  splunk_forwarder_temp_location = local.splunk_forwarder_enabled ? format("gs://%s/splunk-forwarder-temp", var.artifact_bucket) : "gs://placeholder" # reuse release bucket for Dataflow temp files
  splunk_template_path           = local.splunk_forwarder_enabled ? "gs://dataflow-templates/latest/Cloud_PubSub_to_Splunk" : ""
  splunk_forwarder_job_name      = local.splunk_forwarder_enabled ? format("%s-probe-logs-splunk-se", local.env) : ""
}
...
# Log Router sink exporting only probe envelopes to Pub/Sub
resource "google_logging_project_sink" "probe_to_pubsub" {
  name        = "${local.env}-probe-to-pubsub"
...
resource "google_compute_subnetwork" "splunk_dataflow" {
  ...
  ip_cidr_range = var.splunk_network_cidr
}
...
resource "google_dataflow_job" "splunk" {
  ...
  parameters = merge(
    {
      ...
      # Splunk's HEC endpoint uses a cert chain we cannot validate in this env, so we explicitly skip verification per their guidance.
  ...
  lifecycle {
    precondition {
      condition     = var.splunk_hec_url != null && var.splunk_hec_token != null
      error_message = "enable_splunk_forwarder requires splunk_hec_url and splunk_hec_token to be set."
    }
  }
